...
`;console.info(`%c  BAR-CARD \n%c  ${Re("common.version")} 3.4.0    `,"color: orange; font-weight: bold; background: black","color: white; font-weight: bold; background: dimgray");let Ie=class extends he{constructor(){super(...arguments),this._configArray=[],this._stateArray=[],this._animationState=[],this._rowAmount=1}static async getConfigElement(){return document.createElement("bar-card-editor")}static getStubConfig(){return{}}shouldUpdate(e){return function(e,t,i){if(t.has("config")||i)return!0;for(const i of e._configArray)if(i.entity){const r=t.get("hass");if(r){if(r.states[i.entity]!==e.hass.states[i.entity])return!0;continue}return!0}return!1}(this,e,!1)}setConfig(e){if(!e)throw new Error(Re("common.invalid_configuration"));this._config=Ue({animation:{state:"off",speed:5},color:"var(--bar-card-color, var(--primary-color))",columns:1,direction:"right",max:100,min:0,positions:{icon:"outside",indicator:"outside",name:"inside",minmax:"off",value:"inside"}},e),"horizontal"==this._config.stack&&this._config.entities&&(this._config.columns=this._config.entities.length),this._configArray=function(e){const t=[];if(e.entities){for(const i of e.entities)if("string"==typeof i){const r=Ue({},e);delete r.entities;const n=Ue(r,{entity:i});t.push(n)}else if("object"==typeof i){const r=Ue({},e);delete r.entities;const n=Ue(r,i);t.push(n)}}else t.push(e);return t}(this._config),this._rowAmount=this._configArray.length/this._config.columns}render(){return this._config&&this.hass?j`
    ...
    `:j``}_createBarArray(){var e,t,i,r;const n=[];for(let e=0;e<this._configArray.length;e++)(n.length+1)*this._config.columns==e&&n.push(this._config.columns),this._configArray.length==e+1&&n.push(this._configArray.length-n.length*this._config.columns);const a=[];for(let o=0;o<n.length;o++){const s=[];for(let a=0;a<n[o];a++){const n=o*this._config.columns+a,c=this._configArray[n],l=this.hass.states[c.entity];if(!l){s.push(j`
          ...
          `);continue}let d;if(d=c.attribute?l.attributes[c.attribute]:l.state,c.severity&&this._computeSeverityVisibility(d,n))continue;if(c.limit_value&&(d=Math.min(d,c.max),d=Math.max(d,c.min)),!isNaN(Number(d)))if(0==c.decimal)d=Number(d).toFixed(0);else if(c.decimal)d=Number(d).toFixed(c.decimal);else if(!c.attribute){const i=null===(t=null===(e=this.hass.entities)||void 0===e?void 0:e[c.entity])||void 0===t?void 0:t.display_precision;null!=i&&(d=Number(d).toFixed(i))}let h=40;c.height&&(h=c.height);let u,p,m,f="stretch",g="0px 0px 0px 13px",b="right",_="row",y="left",v="height: 100%; width: 2px;";switch(c.direction){case"right":b="right",y="left";break;case"up":g="0px",b="top",_="column-reverse",y="bottom",v="height: 2px; width: 100%;"}switch(m=this._computeSeverityIcon(d,n)?this._computeSeverityIcon(d,n):c.icon?c.icon:l.attributes.icon?l.attributes.icon:(null===(r=null===(i=this.hass.entities)||void 0===i?void 0:i[c.entity])||void 0===r?void 0:r.icon)?this.hass.entities[c.entity].icon:ye(fe(c.entity),d),c.positions.icon){case"outside":u=j`
              <bar-card-iconbar>
                <ha-icon icon="${m}"></ha-icon>
              </bar-card-iconbar>
            `;break;case"inside":p=j`
              <bar-card-iconbar>
                <ha-icon icon="${m}"></ha-icon>
              </bar-card-iconbar>
            `,g="0px";break;case"off":g="0px"}const w=c.name?c.name:l.attributes.friendly_name;let x,S,k,N,A,$,P;switch(c.positions.name){case"outside":x=j`
              <bar-card-name
                class="${c.entity_row?"name-outside":""}"
                style="${"up"==c.direction?"":c.width?`width: calc(100% - ${c.width});`:""}"
                >${w}</bar-card-name
              >
            `,g="0px";break;case"inside":S=j`
              <bar-card-name>${w}</bar-card-name>
            `}switch(k=isNaN(Number(d))?"":c.unit_of_measurement?c.unit_of_measurement:l.attributes.unit_of_measurement,c.positions.minmax){case"outside":N=j`
              <bar-card-min>${c.min}${k}</bar-card-min>
              <bar-card-divider>/</bar-card-divider>
              <bar-card-max>${c.max}${k}</bar-card-max>
            `;break;case"inside":A=j`
              <bar-card-min class="${"up"==c.direction?"min-direction-up":"min-direction-right"}"
                >${c.min}${k}</bar-card-min
              >
              <bar-card-divider>/</bar-card-divider>
              <bar-card-max> ${c.max}${k}</bar-card-max>
            `}switch(c.positions.value){case"outside":$=j`
              <bar-card-value class="${"up"==c.direction?"value-direction-up":"value-direction-right"}"
                >${c.complementary?c.max-d:d} ${k}</bar-card-value
              >
            `;break;case"inside":P=j`
              <bar-card-value
                class="${"inside"==c.positions.minmax?"":"up"==c.direction?"value-direction-up":"value-direction-right"}"
                >${c.complementary?c.max-d:d} ${k}</bar-card-value
              >
            `;break;case"off":g="0px"}let C=this._stateArray[n],E=d;isNaN(Number(d))||isNaN(Number(C))||(E=Number(d),C=Number(C));let T="";E>C?(T="▲","up"==c.direction?this._animationState[n]="animation-increase-vertical":this._animationState[n]="animation-increase"):E<C?(T="▼","up"==c.direction?this._animationState[n]="animation-decrease-vertical":this._animationState[n]="animation-decrease"):this._animationState[n]=this._animationState[n],isNaN(Number(d))&&(T="");const V=this._computeBarColor(d,n);let O,z;switch(c.positions.indicator){case"outside":O=j`
              <bar-card-indicator
                class="${"up"==c.direction?"":"indicator-direction-right"}"
                style="--bar-color: ${V};"
                >${T}</bar-card-indicator
              >
            `;break;case"inside":z=j`
              <bar-card-indicator style="--bar-color: ${V};">${T}</bar-card-indicator>
            `}const M=this._computePercent(d,n),R=this._computePercent(c.target,n);let U=M,L=this._computePercent(c.target,n);L<U&&(U=L,L=M);let I="";c.width&&(f="center",I=`width: ${c.width}`);const q=this._animationState[n];let B="right",H=100*M,F="animationbar-horizontal";"animation-increase-vertical"!=q&&"animation-decrease-vertical"!=q||(B="bottom",F="animationbar-vertical",H=100*(100-M)),s.push(j`
          <bar-card-card
          ...
          >
            ${u} ${O} ${x}
            <bar-card-background
              style="margin: ${g}; height: ${h}${"number"==typeof h?"px":""}; ${I}"
            >
              <bar-card-backgroundbar style="--bar-color: ${V};"></bar-card-backgroundbar>
              ${"on"==c.animation.state?j`
                    <bar-card-animationbar
                      style="animation: ${q} ${c.animation.speed}s infinite ease-out; --bar-percent: ${H}%; --bar-color: ${V}; --animation-direction: ${B};"
                      class="${F}"
                    ></bar-card-animationbar>
                  `:""}
              <bar-card-currentbar
                style="--bar-color: ${V}; --bar-percent: ${M}%; --bar-direction: ${b}"
              ></bar-card-currentbar>
              ${c.target?j`
                    <bar-card-targetbar
                      style="--bar-color: ${V}; --bar-percent: ${U}%; --bar-target-percent: ${L}%; --bar-direction: ${b};"
                    ></bar-card-targetbar>
                    <bar-card-markerbar
                      style="--bar-color: ${V}; --bar-target-percent: ${R}%; ${y}: calc(${R}% - 1px); ${v}}"
                    ></bar-card-markerbar>
                  `:""}
              <bar-card-contentbar
                class="${"up"==c.direction?"contentbar-direction-up":"contentbar-direction-right"}"
              >
                ${p} ${z} ${S} ${A} ${P}
              </bar-card-contentbar>
            </bar-card-background>
